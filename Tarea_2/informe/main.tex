...
\subsubsection*{Limpieza previa}

El dataset de partida es el mismo que en la Tarea~1: 30.213 art\'{\i}culos distribuidos en 26 medios de prensa. Antes de filtrar por medio de prensa, se aplicaron cuatro correcciones sobre problemas de calidad identificados en la Tarea~1:

\begin{enumerate}
    \item \textbf{Valores placeholder en \texttt{title}}: la columna \texttt{title} figura como \texttt{non-null} en el \texttt{DataFrame} de pandas (no tiene \texttt{NaN}), pero en la pr\'{a}ctica algunas filas contienen cadenas sin informaci\'{o}n real, como \texttt{"Not found"}, \texttt{"[Not Found]"}, \texttt{"N/A"} o guiones repetidos. Estos valores no se capturan con un simple \texttt{isnull()}, sino que requieren un chequeo por patr\'{o}n. Se implement\'{o} la funci\'{o}n \texttt{is\_placeholder(val)} basada en una expresi\'{o}n regular que detecta estas cadenas en cualquiera de sus variantes. Los art\'{\i}culos con t\'{\i}tulo placeholder se reportan pero no se eliminan, dado que para la clasificaci\'{o}n de esta tarea se usa el cuerpo del art\'{\i}culo, no el t\'{\i}tulo.

    \item \textbf{Filas con \texttt{article} nulo o placeholder}: 1.176 filas (3,89\% del total) ten\'{\i}an \texttt{article} nulo (\texttt{NaN}), y adicionalmente algunas contendr\'{\i}an cadenas placeholder. Sin texto que procesar, estas filas no son clasificables y se eliminan.

    \item \textbf{Filas con \texttt{publication} nula}: 141 filas (0,47\%) no ten\'{\i}an medio de prensa asignado. Como \texttt{publication} es la variable objetivo $y$ de la clasificaci\'{o}n, estas filas no son utilizables y se eliminan.

    \item \textbf{Duplicados}: se elimin\'{o} toda fila cuya combinaci\'{o}n \texttt{(title, article)} apareciera m\'{a}s de una vez. Art\'{\i}culos repetidos que cayeran en ambos conjuntos (train y test) inflar\'{\i}an artificialmente el \textit{accuracy} del modelo.
\end{enumerate}

La Tabla~\ref{tab:limpieza} resume el efecto de cada paso de limpieza.

\begin{table}[H]
\centering
\caption{Efecto de la limpieza previa sobre el tama\~{n}o del dataset.}
\label{tab:limpieza}
\begin{tabular}{l r r}
\toprule
\textbf{Paso} & \textbf{Filas eliminadas} & \textbf{Filas restantes} \\
\midrule
Dataset original & --- & 30.213 \\
Eliminar \texttt{publication} nula/placeholder & 141 & 30.072 \\
Eliminar \texttt{article} nulo/placeholder & $\sim$1.176 & $\sim$28.896 \\
Eliminar duplicados & [TODO] & [TODO] \\
\midrule
\textbf{Dataset limpio (\texttt{df\_clean})} & \textbf{[TODO] total} & \textbf{[TODO]} \\
\bottomrule
\end{tabular}
\end{table}

\subsubsection*{Funci\'{o}n \texttt{clean\_text}}

Se reutiliz\'{o} la funci\'{o}n \texttt{clean\_text} desarrollada en la Tarea~1, sin modificaciones. La funci\'{o}n aplica los siguientes pasos en orden: eliminaci\'{o}n de la primera l\'{\i}nea (cabecera repetida), conversi\'{o}n a min\'{u}sculas, eliminaci\'{o}n de URLs, eliminaci\'{o}n de puntuaci\'{o}n y caracteres no alfanum\'{e}ricos (reemplazando por espacio para no pegar palabras), eliminaci\'{o}n de n\'{u}meros aislados, y normalizaci\'{o}n de espacios.

A diferencia de lo que se hizo en la Parte~2 de la Tarea~1, aqu\'{\i} \textbf{no se eliminan \textit{stopwords} ni identificadores del medio} en la etapa de limpieza: esas decisiones se toman a nivel del vectorizador (\texttt{CountVectorizer} o \texttt{TfidfVectorizer}) y son parte del an\'{a}lisis de la Secci\'{o}n~\ref{sec:pca}.
...
\midrule
Reuters & $\sim$9.280 & $\sim$62\% \\
The New York Times & $\sim$2.830 & $\sim$19\% \\
CNBC & $\sim$2.610 & $\sim$17\% \\
\midrule
\textbf{Total top~3} & $\sim$\textbf{14.720} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\end{table}

Los tres medios coinciden con los tres primeros del top~5 de la Tarea~1. El dataset resultante presenta un \textbf{desbalance pronunciado}: \textit{Reuters} acumula cerca del 62\% de los art\'{\i}culos, frente al 19\% de \textit{NYT} y el 17\% de \textit{CNBC}. Esto tiene implicancias sobre las m\'{e}tricas de evaluaci\'{o}n que se discuten en la Parte~2.

\subsubsection*{Partici\'{o}n train/test}

El dataset de los tres medios se dividi\'{o} en un conjunto de \textbf{entrenamiento (70\%)} y un conjunto de \textbf{test (30\%)}, utilizando muestreo estratificado por medio de prensa (\texttt{stratify=y}). La estratificaci\'{o}n garantiza que la proporci\'{o}n de art\'{\i}culos de cada medio sea la misma en ambos conjuntos. Se fij\'{o} \texttt{random\_state=42} para reproducibilidad.

\subsection{Verificaci\'{o}n del balance de clases}

\begin{figure}[H]
\centering
\includegraphics[width=0.95\textwidth]{chart_balance.png}
...
El an\'{a}lisis de componentes principales (\textit{PCA}) requiere materializar la matriz como densa para poder centrarla. Con un vocabulario de decenas de miles de palabras y miles de documentos, eso implicar\'{\i}a varios gigabytes en memoria. Por este motivo se utiliz\'{o} \texttt{TruncatedSVD} de scikit-learn, que computa las componentes principales directamente sobre la representaci\'{o}n \textit{sparse} sin densificarla. El resultado es matem\'{a}ticamente equivalente al PCA para estos efectos.

\subsubsection*{Resultados con la configuraci\'{o}n base}

\begin{figure}[H]
\centering
\includegraphics[width=0.85\textwidth]{chart_pca_base.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{chart_pca_configs.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{chart_pca_variance.png}
